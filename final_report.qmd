...
def create_fig2_rotation_comparison():
    """Figure 2: Impact of Coordinate Rotation on Model Performance"""
    rotation_pairs = [
        ('random_forest', 'random_forest_cr'),
        ('xgboost', 'xgboost_cr'),
        ('mixgboost', 'mixgboost_cr'),
        ('gam', 'gam_cr')
    ]
    
    # Prepare data for plotting
    plot_data = []
    for base_key, cr_key in rotation_pairs:
        for dataset in datasets:
            base_res = [r for r in results if r['model'] == base_key and r['dataset'] == dataset]
            cr_res = [r for r in results if r['model'] == cr_key and r['dataset'] == dataset]
            
            if base_res and cr_res:
                plot_data.append({
                    'Algorithm': model_names[base_key].replace('-CR', ''),
                    'Type': 'Standard',
                    'R2': base_res[0]['r2_score'],
                    'Family': base_key
                })
                plot_data.append({
                    'Algorithm': model_names[base_key].replace('-CR', ''),
                    'Type': 'With CR',
                    'R2': cr_res[0]['r2_score'],
                    'Family': base_key
                })
    ...
    sns.barplot(data=df_plot, x='Algorithm', y='R2', hue='Type', ax=ax, palette=['#D3D3D3', '#505050'])
    
    # Coloring the x-axis labels by family
    for i, label in enumerate(ax.get_xticklabels()):
        model_key = [k for k, v in model_names.items() if v == label.get_text()][0]
        label.set_color(get_model_color(model_key))
        label.set_weight('bold')

    ax.set_title('Performance Comparison: Standard vs Coordinate Rotation (CR)')
    ax.set_ylim(0, 1.05)
...
create_fig2_rotation_comparison()
```
...
def create_fig4_real_vs_synthetic():
    """Figure 5: Performance Consistency: Real vs Synthetic Data"""
    real_ds = ['rgealti', 'bdalti', 'bdalti_48', 'rgealti_48']
    
    plot_data = []
    for r in results:
        category = 'Real' if r['dataset'] in real_ds else 'Synthetic'
        plot_data.append({
    ...
    ordered_models = deterministic_models + geostatistical_models + tree_models + advanced_tree_models + gam_models
    df_plot['Model'] = pd.Categorical(df_plot['Model'], categories=[model_names[m] for m in ordered_models], ordered=True)
    df_plot = df_plot.sort_values('Model')

    fig, ax = plt.subplots(figsize=(12, 6))
    sns.boxplot(data=df_plot, x='Model', y='R2', hue='Category', ax=ax, palette=['#4A90E2', '#50E3C2'])
    
    # Color labels
    for i, label in enumerate(ax.get_xticklabels()):
        m_key = ordered_models[i]
        label.set_color(get_model_color(m_key))
        label.set_weight('bold')

    ax.set_title('Performance Range: Real vs Synthetic Datasets')
    ax.set_ylabel('$R^2$ Score')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.show()

create_fig4_real_vs_synthetic()
```
