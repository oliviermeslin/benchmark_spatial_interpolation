...
As mantionned previsouly, we begin our analysis by running all candidate algorithms on "Small" datasets (). This step allows us to evaluate the trade-off between **Training Cost (Time)** and **Predictive Accuracy (RMSE)** before scaling up.
...
1. **Discarded Models:**
* **GAM & GeoRF (Green & Red):** These models show extreme training times even on small data for not a so good perfomance. Running these on datasets 20x larger would require prohibitive compute time without guaranteeing superior performance.
