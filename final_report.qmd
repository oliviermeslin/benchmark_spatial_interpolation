...
| Dataset Name | Origin | Size Category | Structure | Approx. Row Count |
|------------|------------|------------|------------|------------|
| **bdalti** | Real | Large | No Grid | \~7,000,000 |
| **bdalti_48** | Real | Small | No Grid | \~40,000 | 
| **rgealti** | Real | Large | Grid | \~22,000,000,000 |
| **rgealti_48** | Real | Small | Grid | \~1,500,000 |
| **S-G-Sm** | Synthetic | Small | Grid | 10,000 |
| **S-G-Lg** | Synthetic | Large | Grid | 1,000,000 |
| **S-NG-Sm** | Synthetic | Small | No Grid | 10,000 |
| **S-NG-Lg** | Synthetic | Large | No Grid | 1,000,000 |
...
## Small Datasets: What Matters?

As mantioned previsouly, we begin our analysis by running all candidate algorithms on "Small" datasets (). This step allows us to evaluate the trade-off between **Training Cost (Time)** and **Predictive Accuracy (RMSE)** before scaling up.
...
1. **Discarded Models:**
* **GAM & GeoRF (Green & Red):** These models show extreme training times even on small data without providing competitive performance. Running these on datasets 20x larger would require prohibitive compute time without guaranteeing superior performance.
...
2. **Retained Models:**
* **Ensembles (Orange):** RF, XGBoost, and MI-GBT offer the best balance (fast and accurate). We will keep them with their coordinates rotation alternatives. 
* **Deterministic (Blue):** Retained as baselines due to their near-instant speed with strong accuracy.

Consequently, only the **Ensemble** and **Deterministic** families are advanced to the next phase.

## Who is the Better?

### What about scalability ?

Moving from 5,000 to 100,000 points represents a 20x increase in data volume. The table below details the "Time Augmentation" cost. We define the **Scalability Cost** as the ratio of time for large dataset over the time for small dataset.
...
def create_table_time_scalability_v2():
    ...
    for m_key in model_names.keys():
        ...
        times_vlg = [r.get('training_time', 0) for r in results if r['model'] == m_key and r['dataset'] in very_large]
        times_elg = [r.get('training_time', 0) for r in results if r['model'] == m_key and r['dataset'] in extrem_large]
        ...
        if times_sm:
            ...
            table_data.append({
                ...
                'Time (Very Large)': times_vlg,
                'Time (Extrem Large)': times_elg,
...
def create_table_noisy_comparison():
    ...
    key_models = ['knn_3', 'random_forest', 'random_forest_cr', 'xgboost', 'xgboost_cr', 'mixgboost','mixgboost_cr']
...
With computational feasibility established, we analyze the impact of **Coordinate Rotation (CR)** on these large and noisy datasets. The table below compares the standard version of each ensemble algorithm against its CR counterpart for each dataset and show the improvement in term of RMSE but also in term of time training.
