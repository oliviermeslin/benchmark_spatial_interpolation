...
The remaining algorithm are used only for a comparative purpose. This allow us to see how the coordinate rotations based models perform with respect to other existing strategy and isolate the specific impact of coordinate rotation on standard axis-aligned tree algorithms. \### Geometric Interpretation
...
Our benchmark evaluation follows a standardized protocol to ensure fair comparison across all algorithms and datasets. The experimental configuration is summarized in Table X below. For some of the functions that we used, to implement them we built wrappers code, something that could have increase the running time of our algorithms.
...
This is comming first because it will determined, which dataset should be runnned for large data set based on the time spend for executing small datasets.
...
Now that we see that it could be hard for some dataset we are going to try to see if increasing the size of the data could make some algorithm improving fatser than the other. Of course this comparison could be only apply to the one who have run on big and small datasets.
...
Now that we see that large data set could make the R2 varying we are going to try to explain the other differnce by using only small datasets.

### Grid or real what is relevant ?

We have seen that the grid could have some importance in the results
...
A Big table to have synthtic data from the benchmark.
