...
import xgboost 
# lightgbm is excluded from this execution due to environment limitations, 
# but will be included in the final file for the user.


# --- START: Definitions from utils.py (modified to be in one file) ---
epsilon = 1e-10
...
# --- START: Definitions from MIBooster.py (XGBoost parts only for execution) ---
...
# --- END: Definitions from MIBooster.py (XGBoost parts only for execution) ---
...
def generate_synthetic_data(n_samples: int = 2500) -> pd.DataFrame:
    ...
    y = np.linspace(40.0, 42.0, side) 
    x = np.linspace(-75.0, -73.0, side) 
...
def train_mi_gbt(X_train: np.ndarray, y_train: np.ndarray, locs_train: np.ndarray, 
    ...
    """
    Trains the MI-GBT model using XGBoost (MIXGBooster).
    Note: The LightGBM option is not available in the current environment.
    """
    if booster_type != 'XGB':
         print("Warning: Only XGBoost is supported in this execution environment. Defaulting to XGB.")
...
def train_baseline_gbt(X_train: np.ndarray, y_train: np.ndarray, booster_type: str = 'XGB') -> xgboost.XGBRegressor:
    """
    Trains a baseline Gradient Boosted Tree model (XGBRegressor).
    Note: The LightGBM option is not available in the current environment.
    """
    if booster_type != 'XGB':
         print("Warning: Only XGBoost is supported in this execution environment. Defaulting to XGB.")
