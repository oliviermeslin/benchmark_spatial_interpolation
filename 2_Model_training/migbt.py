...
def calc_w_sparse_fast(data, k=5):
    """
    Calcola matrice sparsa dei pesi spaziali (KNN).
    """
    # Algoritmo veloce multi-core
...
def mse_moranI_loss_optimized(y_true, y_pred, w_sparse, lamb):
    """
    FIXED: Ordine argomenti corretto (y_true, y_pred).
    """
    #residui
    resid = y_pred - y_true
...
class MIXGBooster(xgb.XGBRegressor):
    ...
    def fit(self, X, y, locs=None):
        ...
        if self.w_sparse is None:
            print(f"  [MI-GBT] Calcolo W (k={self.k})...", end=" ", flush=True)
